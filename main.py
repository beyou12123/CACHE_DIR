    ...
    async def final_launcher():
        ...
        try:
            ...
            if db_size == 0:
                ...
                # الحالة الثانية: الملف موجود (إقلاع عادي أو إعادة تشغيل بسيطة)
                # إذا الملف موجود، نؤمنه بنسخة احتياطية إضافية قبل أي حركة
                print("💾 [PRE-STARTUP]: تأمين نسخة احتياطية إضافية إلى تليجرام...")
                print("⏳ [WAIT]: جار الرفع يرجى الإنتظار 3 ثوانٍ لتهدئة الاتصال بطلب من تليجرام...")
                
                # انتظار استراتيجي لتجنب الـ Flood Control عند الإقلاع
                await asyncio.sleep(3)
                
                # تنفيذ عملية النسخ الاحتياطي السحابي
                await db_manager.create_backup_to_telegram()
